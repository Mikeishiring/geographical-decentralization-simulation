...
RUN npm ci --workspace explorer --include-workspace-root=false \
 && node -e "const { createRequire } = require('module'); const req = createRequire('/app/explorer/package.json'); if (process.platform === 'linux' && process.arch === 'x64') { req.resolve('@rolldown/binding-linux-x64-gnu'); req.resolve('lightningcss-linux-x64-gnu'); req.resolve('@tailwindcss/oxide-linux-x64-gnu'); req.resolve('@napi-rs/canvas-linux-x64-gnu'); }"
