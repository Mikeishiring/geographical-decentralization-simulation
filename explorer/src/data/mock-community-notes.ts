  ...
  'mock-note-7': {
    ...
    replies: [
      ...
      },
      {
        id: 'reply-7c',
        author: 'Jordan P.',
        body: 'There\'s a third option: make relay infrastructure more geographically distributed. That doesn\'t require protocol changes at all. The SE1 results show relay placement has a first-order effect. Start there before inventing new incentive mechanisms.',
    ...
    replies: [
      {
        id: 'reply-20a',
        author: 'Nadia S.',
        body: 'This reads like initial conditions are destiny, but that\'s too fatalistic. The paper shows paradigm choice still determines the *shape* of concentration even when the starting point is fixed. MSP spreads across more regions than SSP even from the same start. That matters for resilience.',
      ...
      },
      {
        id: 'reply-20b',
        author: 'Ethan Z.',
        body: 'I ran both paradigms from the heterogeneous start in the sim lab. At 10,000 slots, MSP has validators in 14 regions vs SSP\'s 8. Same Gini, completely different fault-tolerance profile. The "initial conditions dominate" claim hides this.',
      ...
      },
      {
        id: 'reply-20c',
        author: 'Alex W.',
        body: 'Ethan, that 14-vs-8 comparison is cherry-picked. Most of those extra MSP regions have <2% of stake. If you threshold at 5% of total stake, MSP has 5 meaningful regions and SSP has 4. The spread is less impressive than it sounds.',
    ...
    replies: [
      {
        id: 'reply-21a',
        author: 'Kira T.',
        body: 'If intentional relay misalignment can temporarily reduce concentration, that\'s a policy lever. Imagine a "geographic diversity relay" deliberately placed in an underserved region. The dip might be temporary in the model, but if you keep moving the relay, you keep refreshing the disruption.',
      ...
      },
      {
        id: 'reply-21b',
        author: 'Danny R.',
        body: 'The "keep moving the relay" idea is creative but impractical. Relay operators need stable infrastructure. Constantly relocating endpoints to chase geographic diversity would degrade reliability for everyone. The cure would be worse than the disease for block production latency.',
      ...
      },
      {
        id: 'reply-21c',
        author: 'Priya N.',
        body: 'You don\'t need to *move* relays — you need to *add* them. The model shows the dip comes from misalignment between existing validators and relay geography. Adding a relay in Asia-Pacific creates that misalignment naturally without disrupting the existing EU/US infrastructure. It\'s additive, not destructive.',
    ...
    replies: [
      {
        id: 'reply-23a',
        author: 'Tomás G.',
        body: 'The SE1 results show relay placement has 0.08 Gini impact. That\'s achievable by convincing 2-3 relay operators to add endpoints in Asia and South America. No protocol change needed, no governance vote, no hard fork. This is the 80/20 solution.',
      ...
      },
      {
        id: 'reply-23b',
        author: 'Ethan Z.',
        body: 'Relay operators optimize for latency to builders, not geographic diversity. A relay in Singapore adds 100ms+ to EU/US builder-relay roundtrips. Unless you compensate relay operators for the throughput cost, they have zero incentive to do this. Who pays?',
      ...
      },
      {
        id: 'reply-23c',
        author: 'Kira T.',
        body: 'The Ethereum Foundation could fund a "geographic diversity relay program" — trivial cost compared to the protocol development budget. Flashbots already runs relays as a public good; this would just extend that mission geographically. The incentive alignment is there if someone organizes it.',
    ...
    replies: [
      {
        id: 'reply-24a',
        author: 'Jordan P.',
        body: 'The higher Gini is real, but Gini doesn\'t capture *where* the concentration is. MSP concentrates in 3-4 distinct clusters (US-East, EU-West, Asia-East), while SSP concentrates in essentially 1-2 (wherever the relay is). A network split that takes out EU-West hurts SSP catastrophically but only clips MSP by ~30%. Which is "more centralized"?',
      ...
      },
      {
        id: 'reply-24b',
        author: 'Alex W.',
        body: 'I plotted the spatial distributions from my sim lab runs. SSP at Gini 0.55 has 72% of stake in 2 regions. MSP at Gini 0.70 has 72% of stake spread across 4 regions. The Gini is higher for MSP but the geographic risk concentration is actually lower. This is a real limitation of scalar metrics.',
      ...
      },
      {
        id: 'reply-24c',
        author: 'Marcus V.',
        body: 'Alex, that\'s a compelling data point but it doesn\'t rescue MSP. A higher Gini means some validators earn dramatically more than others. Even if the top earners are geographically spread, the *economic* concentration creates pressure for smaller validators to exit or consolidate. Geographic spread with economic inequality is still a centralization vector.',
    ...
    replies: [
      {
        id: 'reply-26a',
        author: 'Priya N.',
        body: 'This is the paper\'s most underrated finding. Gamma has a bigger effect on geographic outcomes than the choice between SSP and MSP. In the sim lab, varying gamma from 0.33 to 0.8 moves the Gini by 0.15-0.20 points. Switching paradigms at fixed gamma moves it by ~0.10. Gamma is the bigger lever.',
...
export const MOCK_COMMUNITY_NOTES: readonly Exploration[] = [
  ...
  },
  {
    ...
    anchor: {
      sectionId: 'se3-joint',
      excerpt: 'the temporary dip in concentration appears when SSP starts from today\'s concentrated validator geography.',
    },
    publication: {
      published: true,
      title: 'Intentional relay misalignment is a deployable decentralization tool',
      takeaway: 'The SE3 dip isn\'t a curiosity — it\'s a policy lever. If deliberately adding relays in underserved regions creates temporary decentralization pressure, you can maintain that pressure by continuing to expand relay infrastructure. The dip is "temporary" only if relay geography is static. Make it dynamic and the effect persists.',
      author: 'Priya N.',
      publishedAt: daysAgo(2),
      featured: false,
      editorNote: '',
  ...
  },
  {
    id: 'mock-note-23',
    query: 'Discussion',
    summary: 'Selected text: "decentralizing relays and sources"',
    blocks: [],
    followUps: [],
    model: '',
    cached: false,
    ...
    anchor: {
      sectionId: 'discussion',
      excerpt: 'decentralizing relays and sources, or compensating for latency at the protocol layer.',
    },
    publication: {
      published: true,
      title: 'Relay decentralization is the lowest-hanging fruit by far',
      takeaway: 'Combine SE1 (relay placement moves Gini by ~0.08) with the discussion\'s mitigation list, and the answer is obvious: geographic relay expansion requires no protocol changes, no governance votes, and no hard forks. The Ethereum Foundation could fund a "geographic diversity relay program" for less than the cost of a single Devcon. This is the 80/20 solution the paper points to without explicitly recommending.',
      author: 'Kira T.',
      publishedAt: daysAgo(2),
      featured: true,
  ...
  },
  {
    ...
    publication: {
      published: true,
      title: 'MSP\'s higher Gini is misleading — look at the regional spread instead',
      takeaway: 'Gini measures economic inequality among validators but says nothing about geographic resilience. In my sim lab runs, SSP at Gini 0.55 has 72% of stake packed into 2 regions. MSP at Gini 0.70 has that same 72% spread across 4 regions. Which network survives a regional outage better? Scalar metrics are hiding the spatial structure that matters for security.',
      author: 'Alex W.',
      publishedAt: daysAgo(4),
      featured: false,
      editorNote: '',
  ...
  },
  {
    ...
    publication: {
      ...
      takeaway: 'In the sim lab, varying gamma from 0.33 to 0.80 swings the Gini by 0.15–0.20 points under both paradigms (in opposite directions). Switching paradigms at a fixed gamma moves it by ~0.10. This means the attestation threshold has a *larger* geographic effect than the choice between SSP and MSP. Every EIP that touches timing or attestation thresholds needs a geographic impact assessment.',
      author: 'Priya N.',
      publishedAt: daysAgo(1),
      featured: true,
