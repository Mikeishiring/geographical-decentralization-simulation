...
function MiniChart({
  ...
  return (
    <div className="relative">
      <div className="absolute left-0 top-0 text-[9px] font-medium text-muted tracking-wide px-1.5 py-0.5">
        {yLabel}
      </div>
      <svg
      ...
      >
        ...
        {datasets.map((d, i) => {
          ...
          const hoveredPt = hoverSlot != null
            ? points.reduce<PaperChartPoint | null>((best, p) =>
                best === null || Math.abs(p.x - hoverSlot) < Math.abs(best.x - hoverSlot) ? p : best
              , null)
            : null
            ...
            <g key={d.label}>
              {areaD && (
                <motion.path
                  ...
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  transition={{ ...SPRING_CRISP, delay: 0.05 + i * 0.03 }}
                />
              )}
              <motion.path
                d={pathD}
...
export function PaperChartBlock({ block }: PaperChartBlockProps) {
  ...
  const hoverReadout = hoverSlot != null
    ? datasets.map(d => {
        const giniPt = d.gini.reduce<PaperChartPoint | null>((best, p) =>
          best === null || Math.abs(p.x - hoverSlot) < Math.abs(best.x - hoverSlot) ? p : best, null)
        return { label: d.label, color: d.color, slot: giniPt?.x ?? hoverSlot }
      })
    : []

  return (
    <motion.div
      className="lab-panel overflow-hidden rounded-xl topo-bg card-hover"
      initial={{ opacity: 0, y: 8 }}
      animate={{ opacity: 1, y: 0 }}
      transition={SPRING_CRISP}
    >
      {/* Header */}
      <div className="flex flex-wrap items-start justify-between gap-2 border-b border-rule px-4 py-3">
      ...
      <div className="flex flex-wrap items-center gap-1.5 px-4 pt-3 pb-1">
        ...
        {hoverSlot != null && (
          <span className="ml-auto text-2xs tabular-nums text-muted">
            Slot {hoverReadout[0]?.slot.toLocaleString() ?? hoverSlot.toLocaleString()}
          </span>
        )}
      </div>

      {/* 2x2 grid */}
      <div className="grid grid-cols-2 gap-px bg-rule/30 px-4 pb-4 pt-1">
        {METRICS.map(({ key, yLabel }) => (
          <div
            key={key}
            className={cn(
              'bg-white rounded-lg px-1.5 py-1',
              hoverSlot != null && 'ring-1 ring-accent/5',
            )}
          >
            <MiniChart
              datasets={datasets}
      ...
      {hoverSlot != null && (
        <div className="border-t border-rule px-4 py-2">
          <div className="flex flex-wrap gap-x-4 gap-y-1 text-2xs tabular-nums">
            {datasets.map(d => {
              const vals = METRICS.map(({ key, label }) => {
                const pt = d[key].reduce<PaperChartPoint | null>((best, p) =>
                  best === null || Math.abs(p.x - hoverSlot) < Math.abs(best.x - hoverSlot) ? p : best, null)
                return { label, value: pt?.y ?? 0 }
              })
              return (
                <div key={d.label} className="flex items-center gap-2">
                  ...
                  {vals.map(v => (
                    <span key={v.label} className="text-text-primary">
                      {v.label} <span className="font-medium">{formatNum(v.value)}</span>
                    </span>
                  ))}
                </div>
              )
