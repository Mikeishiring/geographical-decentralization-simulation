...
const ASSISTANT: StudyPackage['assistant'] = {
  ...
  workflows: [
    {
      ...
      description: 'Start with a paper-level explanation of why geography matters before drilling into one figure family.',
      prompt: 'Can you explain the project and its main mechanism?',
      mode: 'ask',
      routeHint: 'orientation',
      badge: 'Overview',
      outputs: ['overview page', 'core contrast', 'next questions'],
      bestFor: ['First-time readers', 'Mechanism questions', 'Context before diving into Results'],
    },
    {
      id: 'compare-paper-results',
      ...
      fields: [
        {
          ...
          options: [
            ...
            { value: 'proposal_times', label: 'Proposal time', promptValue: 'proposal time' },
            { value: 'attestations', label: 'Attestations', promptValue: 'attestations' },
          ],
        },
        {
    ...
    {
      id: 'inspect-a-sweep',
      title: 'Inspect a sweep',
      description: 'Stay inside one experiment family and compare the sweep settings with a compact query surface.',
      prompt: 'Show me the higher-gamma runs sorted by final Gini.',
      promptTemplate: 'Show me the {{family}} runs sorted by final {{metric}}.',
      structuredQueryTemplate: {
        viewId: '{{family}}',
        dimensions: ['evaluation', 'paradigm', 'result'],
        metrics: ['{{metric}}'],
      ...
      fields: [
        {
          id: 'family',
          label: 'Sweep family',
          defaultValue: 'gamma-sweep',
          options: [
            { value: 'gamma-sweep', label: 'Higher gamma', promptValue: 'higher-gamma' },
            { value: 'slot-time-runs', label: 'Shorter slots', promptValue: 'slot-time' },
            { value: 'source-placement-runs', label: 'Source placement', promptValue: 'source-placement' },
          ],
        },
        {
          id: 'metric',
  ...
  queryViews: [
    {
      id: 'published-runs',
      title: 'Published Runs Leaderboard',
      description: 'Rank or tabulate all published paper runs using the final snapshot metrics.',
      surface: 'leaderboard',
      prompts: ['Show me a table of published runs sorted by final Gini.'],
      aliases: ['published runs', 'all runs', 'leaderboard', 'results catalog', 'final gini', 'final hhi'],
      bestFor: [
        'Ranking the full published catalog by one metric',
        'Comparing paradigms across multiple paper scenarios',
        'Finding the most concentrated or slowest runs quickly',
      ],
      ...
      constraints: {
        ...
        filters: {
          ...
          paradigm: ['External', 'Local'],
        },
      },
      executionHints: [
        {
          label: 'Lead with one metric',
          description: 'Start with one ranking metric, then add only a few columns so the leaderboard stays readable.',
        },
        {
          label: 'Prefer final snapshot',
          description: 'Use the final slot by default unless the reader explicitly asks for the starting conditions.',
        },
      ],
    },
    {
      id: 'gamma-sweep',
      title: 'Gamma Sweep Table',
      description: 'Tabulate the attestation-threshold sweep with gamma, paradigm, and concentration metrics side by side.',
      surface: 'parameter-sweep',
      prompts: ['Show me the higher-gamma runs sorted by final Gini.'],
      ...
      defaultDimensions: ['evaluation', 'paradigm', 'gamma'],
      defaultMetrics: ['gini', 'attestations'],
      defaultOrderBy: 'gini',
      defaultOrder: 'desc',
      defaultLimit: 8,
      filterPreset: {
      ...
      constraints: {
        ...
        filters: {
          evaluation: ['SE4-Attestation-Threshold'],
          paradigm: ['External', 'Local'],
          result: ['gamma_0.3333', 'gamma_0.5', 'gamma_0.6667', 'gamma_0.8'],
        },
      },
      executionHints: [
        {
          label: 'Keep it sweep-shaped',
          description: 'Prefer gamma plus paradigm columns and sort by one outcome so the parameter sweep reads top to bottom.',
        },
        {
    ...
    {
      id: 'slot-time-runs',
      title: 'Slot Time Comparison Table',
      description: 'Compare the baseline and shorter-slot published runs in one compact table.',
      surface: 'comparison-table',
      prompts: ['Show me the slot-time runs sorted by proposal time.'],
      aliases: ['slot time', 'shorter slots', 'eip-7782', 'se4b'],
      dashboardIds: ['slot-time-comparison'],
      bestFor: [
        'Comparing 12-second versus 6-second slots',
        'Ranking slot-time runs by latency or fairness pressure',
        'Showing what changes and what stays flat under EIP-7782',
      ],
      defaultDimensions: ['evaluation', 'paradigm', 'result'],
      defaultMetrics: ['proposal_times', 'gini'],
      defaultOrderBy: 'proposal_times',
      defaultOrder: 'desc',
      defaultLimit: 6,
      constraints: {
        ...
        filters: {
          ...
          paradigm: ['External', 'Local'],
          result: ['cost_0.002', 'delta_6000_cutoff_3000'],
        },
      },
      executionHints: [
        {
          label: 'Keep both slot families visible',
          description: 'A useful slot-time answer usually keeps baseline and EIP-7782 rows in the same compact table.',
        },
        {
          label: 'Lead with latency or fairness',
          description: 'Proposal time and Gini are the strongest first-pass ranking metrics for this family.',
        },
      ],
    },
    {
      id: 'source-placement-runs',
      title: 'Source Placement Sweep',
      description: 'Lay out the aligned and misaligned source-placement runs as a compact results table.',
      surface: 'parameter-sweep',
      prompts: ['List the source-placement runs and compare their final Gini.'],
      aliases: ['source placement', 'aligned', 'misaligned', 'se1'],
      dashboardIds: ['source-placement-effects'],
      bestFor: [
      ...
      constraints: {
        ...
        filters: {
          evaluation: ['SE1-Information-Source-Placement-Effect'],
          paradigm: ['External', 'Local'],
          result: ['latency-aligned', 'latency-misaligned'],
        },
      },
      executionHints: [
        {
        ...
        },
        {
          ...
          description: 'The result column is the cleanest axis for this sweep; avoid cluttering the table with too many extra fields.',
        },
      ],
    },
  ],
